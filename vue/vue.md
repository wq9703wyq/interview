...
# 13. key 的作用
   ...
   4. 所以对于动态节点，特别是类似 `v-for` 这种会产生大量动态节点的场景，需要对所有子节点设置 key 且 key 值不应该使用更新后顺序不会发生变化的字符串数组，若列表中使用 index 来做 key 值，并且列表子项依赖于响应式数据，那么最终结果视图则不会有异常，因为无论怎么打乱key序列，实际上都会因为子项的props发生了变化而导致第二次render，得到了我们想要的结果；但是如果子项内容都是静态，在删除首项内容的时候会发生反而是尾项被删除的情况；但无论如何，只要使用了 index 来做 key 值，当列表子项顺序发生变化时都会使 diff 失效，额外消耗性能
...
# 20. vue3 新特性
   1. vue3 一个对开发最直接的新特性便是 Composition Api，也就是组合式api，它的一个最显著的优点便是解决了 mixins 的缺点，没有以前mixins的命名冲突、数据来源不清晰等问题，Composition Api中可以像使用Lodash一样引用你封装好的复用逻辑，同时这也催生出像vueUse 这样的工具函数库
   2. 还有一个改变了组件封装习惯的新特性 Teleport，这个特性主要针对 dom 结构比较复杂的组件，例如模态框、通知栏，像是模态框以前如果要在同一个页面多个block中应用，一般都是将多个模态框组件传入到需要用到的block中，但是使用 Teleport 包裹组件则可以动态传入目标 block 的类名或者 id 直接将组件传送到目标 Block
   3. 在 css 上也新提供了 ::v-slot 和 ::v-global 两个伪类，2 中 slot 中的样式同时会受到父子组件样式的影响，现在如果子组件要修改 slot 的样式则必须显示定义 ::v-slot，2中的路由页面或者子组件要定义全局样式则必须另外定义 \<style> 标签，并且在项目加载了这个 \<style> 的内容后由于标签不带 scoped 所以会一直影响全局样式，但是 ::v-global 可以写在 scoped 标签中；另外 vue3 也提供了动态变量控制 style 文件的css变量，要知道功能类似的浏览器特性目前也还在测试中
   4. 另外还有 vue3 虚拟dom与diff的重写，对大型页面渲染更好地支持